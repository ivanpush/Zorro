# ZORRO Backend - Assembler Implementation
...
### Priority (Dedup)
When two findings have overlapping anchors:
```
adversary (1) beats rigor (2) beats clarity (3)
```
Lower priority number = higher actual priority = wins.

### Presentation Order (Output)
Final output sorted for reading:
```
clarity (1) → rigor (2) → domain (3) → adversary (4)
```
User sees clarity issues first, then rigor, then adversarial critiques.

### Overlap Detection
Two anchors overlap if same paragraph AND any of:
1. Exact text match
2. One is substring of other
3. >50% word overlap
4. Contiguous word sequence shared (n-gram match)
