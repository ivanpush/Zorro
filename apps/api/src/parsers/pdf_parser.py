            ...
            if is_section:
                ...
                if current_paragraph["text"]:
                    para = self._create_paragraph(
                        current_paragraph,
                        para_index,
                        sections[current_section_idx].id if current_section_idx < len(sections) else None
                    )
                    paragraphs.append(para)
                    section_para_ids.append(para.id)
                    para_index += 1
                ...
                if i < len(parts) - 1:
                    ...
                    if sentence_text:
                        sentences.append(Sentence(
                            id=f"{para_id}_s_{sentence_index:03d}",
    ...
    def _extract_figures(
                    ...
                    for para in page_paragraphs:
                        if self.figure_pattern.match(para.text):
                            ...
                            if para.bounding_box:
                                ...
                                if vertical_distance < 50:  # Within 50 points
                                    caption = para.text
                                    caption_para_id = para.id
                                    break

                    figure = Figure(
                        id=f"fig_{fig_index:03d}",
                        index=fig_index,
                        caption=caption,
                        caption_paragraph_id=caption_para_id,
                        page_number=page_num,
                        after_paragraph_id=page_paragraphs[-1].id if page_paragraphs else None,
        ...
        for para in paragraphs:
            if self.figure_pattern.match(para.text):
                ...
                existing = any(
                    f.caption_paragraph_id == para.id
                    for f in figures
                )
                if not existing:
                    ...
                    figure = Figure(
                        ...
                        index=fig_index,
                        caption=para.text,
                        caption_paragraph_id=para.id,
                        page_number=para.page_number,
                        after_paragraph_id=paragraphs[para.index - 1].id if para.index > 0 else None,
